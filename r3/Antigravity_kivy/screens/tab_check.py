...
class CheckTab(BoxLayout):
    ...
    def _get_tag_info(self, rfid_code):
        ...
        if status == 'not_found':
            ...
            self.show_inventory_info(data.get('inventory'))
        elif status == 'found_location':
            self.show_location_info(data.get('location'), data.get('items'))
    ...
    def show_inventory_info(self, inv):
        if not inv:
            self.ids.result_container.add_widget(Label(text="พบ Tag แต่ไม่พบรายละเอียดสินค้า", font_name='Kanit', size_hint_y=None, height=40))
            return
    ...
    def show_location_info(self, loc, items):
        if not loc:
            self.ids.result_container.add_widget(Label(text="พบ Tag แต่ไม่พบรายละเอียดสถานที่", font_name='Kanit', size_hint_y=None, height=40))
            return
        ...
        if items:
            ...
            for i, item in enumerate(items, 1):
                ...
                item_lbl = Label(text=item_text, font_name='Kanit', size_hint_y=None, height=30, halign='left')
                item_lbl.bind(size=item_lbl.setter('text_size'))
                self.ids.result_container.add_widget(item_lbl)
