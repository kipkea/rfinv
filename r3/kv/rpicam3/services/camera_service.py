...
class CameraService:
    def __init__(self):
        ...
        self.is_barcode_detection_enabled = False
    ...
    def toggle_barcode_detection(self, enable):
        """Enables or disables real-time barcode detection."""
        self.is_barcode_detection_enabled = enable
        Logger.info(f"CameraService: Barcode detection set to {enable}")
    ...
    def get_processed_frame(self) -> np.ndarray or None:
        ...
        # 6. Barcode/QR Code Detection
        barcode_results = []
        if self.is_barcode_detection_enabled:
            barcode_results = self.barcode_service.detect_and_decode(frame)
            self.barcode_results = barcode_results # Store for potential use
            
            # Draw bounding boxes and text on the frame
            for result in barcode_results:
                (x, y, w, h) = result['rect']
                cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
                text = f"{result['type']}: {result['data']}"
                # Ensure text is drawn within bounds
                text_pos = (x, y - 10) if y - 10 > 10 else (x, y + h + 20)
                cv2.putText(frame, text, text_pos, cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
        
        return frame, barcode_results
