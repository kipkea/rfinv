...
import numpy as np
from config import TEMP_DIR, PREVIEW_WIDTH, PREVIEW_HEIGHT, IMAGE_FORMAT, VIDEO_FORMAT
...
class CameraService:
    ...
    def capture_image(self, user_filename: str = None) -> str or None:
        """
        ...
        filename = self._generate_unique_filename(user_filename, IMAGE_FORMAT)
    ...
    def start_recording(self, user_filename: str = None) -> str or None:
        ...
        # Generate unique filename
        filename = self._generate_unique_filename(user_filename, 'avi') # Use AVI for raw capture
        file_path = os.path.join(self.temp_dir, filename)
    ...
    def stop_recording(self) -> str or None:
        ...
        file_path = self.video_writer.getFilename()
    ...
    def _generate_unique_filename(self, user_filename: str or None, extension: str) -> str:
        ...
        if user_filename:
            ...
            if sanitized_name:
                return f"{sanitized_name}_{timestamp}_{unique_id}.{extension}"
        
        return f"capture_{timestamp}_{unique_id}.{extension}"
