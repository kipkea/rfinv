...
LabelBase.register(name='Kasemanee', fn_regular='THSarabunNew.ttf')
...
class MultiLangKeyboard(BoxLayout):
    ...
    def __init__(self, target_input, **kwargs):
        ...
        self.language_btn = Button(text='สลับเป็น: ภาษาอังกฤษ', size_hint_y=None, height=70, font_size=30, font_name="Kasemanee")
        self.language_btn.bind(on_release=self.toggle_language)
        self.add_widget(self.language_btn)

        # ใช้ GridLayout สำหรับจัดปุ่ม
        self.keyboard_layout = GridLayout(cols=4, padding=10, spacing=5)  # เปลี่ยนเป็น 4 คอลัมน์สำหรับเครื่องคิดเลข
    ...
    def update_keyboard(self):
        ...
        else:  # ตัวเลข (จัดรูปแบบให้เหมือนเครื่องคิดเลข)
            keys = ['7','8','9','+',
                    '4','5','6','-',
                    '1','2','3','×',
                    '0','.','=','÷']

        # เพิ่มปุ่ม Shift และ Caps Lock
        self.keyboard_layout.add_widget(self.create_button('Shift', self.toggle_shift))
        self.keyboard_layout.add_widget(self.create_button('Caps Lock', self.toggle_caps_lock))

        for key in keys:
            btn = Button(text=key, font_size=30, font_name="Kasemanee", size_hint_y=None, height=70)
            if key == 'Space':
                btn.text = 'เว้นวรรค'
                ...
                btn.bind(on_release=lambda x: self.calculate_result())
            else:
                btn.bind(on_release=lambda x, t=key: self.insert_text(t))
            self.keyboard_layout.add_widget(btn)

    def create_button(self, text, action):
        btn = Button(text=text, font_size=30, font_name="Kasemanee", size_hint_y=None, height=70)
    ...
    def toggle_caps_lock(self, instance):
    ...
    def calculate_result(self):
        try:
            # ใช้ eval เพื่อคำนวณผลลัพธ์จากข้อความ
            result = eval(self.target.text)
            self.target.text = str(result)
        except Exception as e:
            self.target.text = "Error"

class MainLayout(BoxLayout):
    def __init__(self, **kwargs):
        super().__init__(orientation='vertical', spacing=10, padding=20, **kwargs)

        self.text_input = TextInput(font_size=40, multiline=True, hint_text='พิมพ์ข้อความที่นี่...', font_name="Kasemanee")
        self.add_widget(self.text_input)

        self.keyboard = MultiLangKeyboard(target_input=self.text_input, size_hint_y=0.6)
        self.add_widget(self.keyboard)

        self.exit_btn = Button(text='ออกจากโปรแกรม', size_hint_y=None, height=80, font_size=30, background_color=(1, 0, 0, 1), font_name="Kasemanee")
